...
**虚拟路径MES系统**（Manufacturing Execution System，制造执行系统）是一款面向离散制造业的开源数字化解决方案，以微服务架构覆盖车间生产完整业务链条，帮助制造企业低成本落地数字化底座。

**核心能力**

- **生产管理**：工单全生命周期管理，覆盖创建、排产、报工、关闭的完整闭环，配套生产调度看板与 APS 排产
- **工艺管理**：工艺模板、工序步骤、参数配置与校验，确保生产过程按标准执行
- **质量管理**：质检记录、待检判定、质量追溯与 SPC 统计过程控制，8 条判异规则实时预警
- **设备监控**：MQTT 设备接入与 Kafka 消息队列实现 10000+ 条/秒数据吞吐，3D 数字孪生实时映射车间设备状态
- **智能分析**：LightGBM/XGBoost 质量预测模型与 GLM-4 驱动的 AI 生产助理，用自然语言完成数据查询、告警诊断与报表生成
