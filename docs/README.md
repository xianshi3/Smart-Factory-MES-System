# 智能工厂 MES 系统 - 文档中心

## 文档目录

### 入门指南

| 文档 | 说明 |
|------|------|
| [README.md](../README.md) | 项目简介、快速开始、访问地址 |
| [DEVELOPMENT.md](./DEVELOPMENT.md) | 开发文档、技术栈、API接口 |

### 技术设计

| 文档 | 说明 |
|------|------|
| [DESIGN.md](./DESIGN.md) | 系统架构、技术选型、数据库设计 |

### 更新记录

| 文档 | 说明 |
|------|------|
| [CHANGELOG.md](./CHANGELOG.md) | 版本更新日志 |

### 技术博客

| 文档 | 说明 |
|------|------|
| [MyBatis-Plus-逻辑删除与唯一约束冲突问题](./MyBatis-Plus-逻辑删除与唯一约束冲突问题.md) | 逻辑删除与唯一约束冲突问题深度解析 |

### 功能设计

| 文档 | 说明 |
|------|------|
| [DOC_DELETE.md](./DOC_DELETE.md) | 删除功能设计文档 |

### 部署运维

| 文档 | 说明 |
|------|------|
| DOC_DEPLOY.md | 部署文档（待创建） |
| DOC_DOCKER.md | Docker 使用指南（待创建） |

### 模块文档

| 文档 | 说明 |
|------|------|
| [mes-ai-service/README.md](../mes-ai-service/README.md) | AI 服务说明 |
| [mes-device-simulator/README.md](../mes-device-simulator/README.md) | 设备模拟器说明 |

---

## 快速导航

- **启动项目**: 双击 `start-all.bat` 或参考根目录 `README.md`
- **开发指南**: 参考 `DEVELOPMENT.md`
- **删除功能**: 参考 `DOC_DELETE.md`
- **清理缓存**: 运行 `start-all.bat` 选择 `[8] Clean`

---

## 文档贡献

如有更新文档，请同步更新本 index 文件。

---

*最后更新: 2026-04-06*